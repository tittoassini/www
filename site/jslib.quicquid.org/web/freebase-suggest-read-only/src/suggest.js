...
function SuggestControl() {
	...
	// options::Object the options passed to the suggest control
        // txt::String     text entered by user
        // cb::Function    callback to call with results, format is {status:'200 OK',results:[obj1,..]}
	onLoad: function (options,txt,cb) {
	  ...
	  param.mql_output='[{"guid": null,"id": null,"name": null,"type": [],"/common/topic/image": [{"guid": null,"id": null,"type": "/common/image","optional": true,"limit": 1,"index": null,"sort": "index"}]}]';
	  $.ajax({
		type: "GET",
...
http://www.freebase.com/api/service/search?query=w&type=/people/person&mql_output=[{%22id%22:null,%22/people/person/gender%22:null}]
...
p.list_load = function(input) {//fb.log("list_load", input);
    ...
    options.onLoad(options,txt,this.delegate("list_receive", [input, txt]));
...
fb.QuicQuidControl = function(handler) {
  ...
  */
  this.handle = function(data) {handler(data);}
};


// inheritance: prototype/constructor chaining
fb.QuicQuidControl.prototype = new fb.InputControl();
fb.QuicQuidControl.prototype.constructor = fb.QuicQuidControl;

var qc = fb.QuicQuidControl.prototype;

/*
fb.state_starting.prototype.enter = function(data) {
      fb.log("state_getting.enter", data);
      //topic.setTopic($("#userInput").val());
    };
*/

$.fn.qqSuggest = function(handler) {
  var options = {};
  $(this)._freebaseInput(new fb.QuicQuidControl(handler), options);
  return $(this);
};
