goog.provide('quid2.ui.view.Service.Org.Quid2.App.Types');
...
quid2.ui.view.Service.Org.Quid2.App.Types.AppModule.displaySource = function(ctx,obj) {
  ...
  function saveSource(newSourceCode) {
      return ctx.cop("Saving new source code",app.Last.value.updateSource.call(obj.relName,newSourceCode));
  };
  ...
  function eval(expr) {
    ...
    /*var evaluator = quid2.modules.getFunction(ref).pipe(function (f) {
    if (q.isUndefined(f))
      return new p.String("Error: " + expr.id + " is undefined.");
    return f.call();
    });*/
