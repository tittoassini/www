...
data Config c = Config   -- runMode::RunMode
                         -- pname :: String
...
data UserCmd cfg =
  UserCmd { mConfig :: Maybe cfg, mService :: Maybe ServiceCmd }
  deriving (Eq, Show)

data ServiceCmd = Start
                | Restart
                | Stop
  deriving (Eq, Show)
...
doMain name realMain mode userCmd = do
  ...
  runLog convText
         (logCfg [(fromString name, Debug)])
         [handler text (file logFile)]
    $ handle (\(e :: SomeException) -> fatalErr (show e)) (realMain cfg)

-- convText ∷ Converter Text
convText = textFmt defaultTimeFormat "{level} {component}:{scope}> {message}"
...
--        else fileHandler logFile DEBUG
--          >>= \h -> return
--          $ setFormatter
--            h
...
--   warningM "Service" $ "Starting: " ++ name
    --print mode
    -- Setup log
